...
const MovieInfo = props => {
	...
	useEffect(() => {
		const { url } = movieIdPath;
		//call action
		if (url) {
			getMovieDetails(url);
			getRecommendationMovies(url);
		}
	}, [paramsId]);
						...
						<div className="movie__details--box">
							...
							<h5 className="movie__details--runtime">
								...
								{movieDetails.runtime
									? getRuntime(movieDetails)
									: `${movieDetails.number_of_seasons} Seasons ${movieDetails.number_of_episodes} Episodes`}
